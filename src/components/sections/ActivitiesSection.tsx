...
function Activity({ title, description, image, imageAlt }: ActivityProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  return (
    <div className="relative">
      <div className="flex flex-col h-full bg-white rounded-[2rem] overflow-hidden shadow-[0_-8px_15px_-3px_rgba(0,0,0,0.05),0_8px_15px_-3px_rgba(0,0,0,0.1)] relative z-10 transition-all duration-300 hover:shadow-2xl hover:translate-y-[-2px]">
        ...
        <div className="p-8">
          ...
          <ul className={`space-y-4 transition-all duration-300 overflow-hidden md:h-auto ${isExpanded ? 'h-auto' : 'h-0 md:h-auto'}`}>
            {description.map((item, index) => (
              <li key={index} className="flex gap-4 items-start">
                <div className="w-2 h-2 rounded-full bg-beige-400 mt-2.5 shrink-0" />
                <p className="text-gray-600 font-light text-base">{item}</p>
              </li>
            ))}
          </ul>
        </div>
